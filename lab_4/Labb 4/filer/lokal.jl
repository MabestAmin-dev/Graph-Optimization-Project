...
# Define variables
#----------------------------------------------------------------
@variable(LOKAL, 0 <= x[1:m, 1:n])
@variable(LOKAL, 0 <= y[1:m] <= 1, Int)
...
# Add Constraints
#----------------------------------------------------------------
@constraint(LOKAL, con1[i in 1:m], 
    sum(x[i, j] for j in 1:n) <= s[i] * y[i])

@constraint(LOKAL, con2[i in 1:n], 
    sum(x[j, i] for j in 1:m) == d[i])

# @constraint(LOKAL, con3[i in 1:n, j in 1:m], x[j, i] <= d[i] * y[j])
#-----------------------------------------------------------------
