...
#-------------------------------------------------------------------------------
@constraint(SUDO, fix1[i in 1:9, k in 1:9], 
    sum(x[i, j, k] for j in 1:9) == 1)
#-------------------------------------------------------------------------------
@constraint(SUDO, fix2[j in 1:9, k in 1:9], 
    sum(x[i, j, k] for i in 1:9) == 1)
#-------------------------------------------------------------------------------
@constraint(SUDO, fix3[i in 1:9, j in 1:9],
    sum(x[i,j,k] for k in 1:9) == 1) 
#-------------------------------------------------------------------------------
@constraint(SUDO, fix4[x_range in 1:3, y_range in 1:3, k in 1:9],
    sum(x[i, j, k] for i in x_range*3-2:x_range*3, j in y_range*3-2:y_range*3) == 1)
#-------------------------------------------------------------------------------
@constraint(SUDO, fix5[i in 1:9, j in 1:9; given[i,j] > 0],
    x[i, j, given[i, j]] == 1 )
#--------------------------------------------------------------------------------
